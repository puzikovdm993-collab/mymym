...
let radialMenuConfig = {
    ...
    radius: 90,
    items: [
        { id: 'undo', icon: 'icon-undo', label: 'Отменить', action: 'undo', enabled: false },
        ...
        // Инструменты
        { id: 'profile', icon: 'CurvesEffectIcon', label: 'Профиль', action: 'profiles', enabled: true },
        { id: 'history', icon: 'icon-history', label: 'История', action: 'history', enabled: false }
    ]
};
...
function createRadialMenu() {
    ...
    centerBtn.onclick = hideRadialMenuMove;
...
function handleRadialMenuAction(action) {
    switch (action) {
        ...
        case 'median':
            ...
            alert('Вставка: функция недоступна');
            // applyMedianFilter();
            break;
        case 'approximation':
        ...
        case 'roundSearch':
            ...
            break;
        case 'profiles':
            if (typeof setTool === 'function') setTool('profile');
            break;
        default:
    ...
    <div class="modal-content radial-menu-config-modal">
        ...
        <div class="modal-body radial-menu-config-body">
            ...
            <div class="config-panel">
                ...
                <div class="panel-section">
                    ...
                    <div class="panel-content orders-list" id="ordersList">
                        <!-- Кнопки фильтров будут добавлены динамически -->
                    </div>
                </div>
        ...
        <div class="action-panel">
            ...
            <div class="action-buttons">
                <button class="modal-btn" onclick="closeRadialMenuConfigModal()">Отменить</button>
                <button class="modal-btn primary" id="saveButton" onclick="saveRadialMenuConfigFromModal()" disabled>Сохранить</button>
            </div>
        </div>
...
function renderOthersList() {
    ...
    const other = modalState.currentConfig.items.filter(item => 
        defaultOther.some(f => f.id === item.id)
    );
    console.log(modalState.currentConfig.items)
    
    other.forEach((item, index) => {
        const otherItem = createToolItem(item, index, 'other');
...
`;
    ...
    if (itemCount > 0) {
        ...
        enabledItems.forEach((item, index) => {
            ...
            svgContent += `
    <!-- Элемент ${item.label} -->
    <g transform="translate(${x}, ${y}) translate(-15, -15)">
        <rect width="25" height="25" fill="var(--modal-bg)" rx="2"/>
        <svg x="2" y="2" width="22" height="22" viewBox="0 0 25 25">
        ...
        if (existingConfig) {
            ...
            if (currentRadialConfig) {
                ...
                modalState.originalConfig = {
                    enabled: true,
                    radius: 90,
                    items: [...defaultTools, ...defaultFilters, ...defaultOther]
                };
