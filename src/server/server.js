...
function showServerCommander(mode = 'open', callback = null) {
    ...
    loadPanelListInServer('left');
    loadPanelListInServer('right');

    updateActivePanelHighlight(); // <-- добавить здесь
}
// Загрузить список файлов для указанной панели
async function loadPanelListInServer(panel) {
    console.log('loadPanelListInServer');
    ...
    try {
        ...
        datassss = await response.json();
        if (!Array.isArray(datassss)) throw new Error('Некорректный ответ сервера');

        const allFiles = datassss.map(item => ({
            name: item.name || '',
            size: item.size || 0,
    ...
    files.forEach(file => {
        ...
        html += `
            <tr class="file-row ${selectedClass}" 
                ...
                ondblclick="onItemDblClick1('${file.name}', false)">
                <td>📄 ${fileName}</td>
...
function onItemDblClick1(panel, folderPath){
    console.log(panel);
    // closeLoadMethodModal();
    console.log('showServerCommander("open", (filePath) => {');
    loadImageFromServer(panel); 
...
function refreshBothPanelsToServer() {
    loadPanelListInServer('left');
    loadPanelListInServer('right');
}
...
const DB_NAME = 'ProjectDataDB';           // Объявление константы с именем базы данных IndexedDB. Используется для идентификации БД в браузере.
const DB_VERSION = 1;                   // Версия схемы базы данных.При изменении структуры (добавление хранилищ) версию нужно увеличивать, чтобы сработал обработчик onupgradeneeded.
const STORE_NAME = 'ProjectData';          // Имя основного хранилища (object store) для хранения актуальных данных JSON. Сюда будет записываться последнее успешное состояние данных.
let db = null;                          // Переменная для хранения ссылки на объект открытой базы данных (IDBDatabase). Инициализирована null, так как БД еще не открыта.
let currentData = {};                   // Переменная для хранения текущих данных приложения в памяти. Используется как кэш для быстрого доступа без запросов к IndexedDB. Инициализирована пустым объектом.
let isMinIOAvailable = true;            // Флаг, указывающий на доступность хранилища MinIO (или аналогичного S3-совместимого хранилища). Используется для определения стратегии сохранения данных (на сервере или только локально).
let syncQueue = [];                     // Массив-очередь для хранения операций синхронизации. Позволяет обрабатывать запросы на запись в порядке их поступления, предотвращая гонки (race conditions).
let isProcessingQueue = false;          // Флаг, указывающий, что текущий процесс обработки очереди синхронизации уже запущен. Предотвращает параллельную обработку очереди несколькими вызовами одновременно.
...
function loadToIndexedbData() {
    console.log('loadToIndexedb');
    return new Promise((resolve, reject) => {
...
async function loadFromIndexedDB() {
    try {
        const data = await loadToIndexedbData();
        console.log(data);
...
function saveToIndexedb() {
    console.log('saveToIndexedb');
    return new Promise((resolve, reject) => {
        const transaction = db.transaction([STORE_NAME], 'readwrite');
        const store = transaction.objectStore(STORE_NAME);
        const request = store.put({ id: 'current', data: null, timestamp: new Date() });
...
async function loadMatrix123123() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/load_matrix_binary/${PROJECT_ID}`, {
            method: 'POST',
            headers: {
...
function loadProjectFromMINIO(){

}

function saveProjectToMINIO(){
