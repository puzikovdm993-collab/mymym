...
async function loadProjectsFromMinIO() {
    try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/v1/projects`, { method: 'GET' });
...
async function saveProjectToMinIO(project) {
    try {
        ...
        // const res = await fetch(`${API_BASE_URL}/save_project/${projectId}`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ project }) });
        // const res = await fetch(`${API_BASE_URL}/api/projects`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ project }) });
        const res = await fetch(`${API_BASE_URL}/api/v1/projects`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ project }) });
        const data = await res.json();
        return data.success;
    } catch(e) { return false; }
}
async function deleteProjectFromMinIO(projectId) {
    try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/v1/projects/${projectId}`, { method: 'DELETE' });
        const data = await res.json();
        return data.success;
    } catch(e) { return false; }
}
...
// Объявление функции, принимающей идентификатор выбранного проекта
function selectProject(projectId) {

    // Поиск проекта в массиве projects по его ID
    // Используем стрелочную функцию для условия поиска
    const p = projects.find(project => project.id === projectId);

    // Если проект с таким ID не найден, прекращаем выполнение функции
    if (!p) return;
    
    // Сохраняем ID активного проекта в глобальной (или внешнем контексте) переменной
    activeProjectId = projectId;

    // Перерисовываем боковую панель (список проектов)
    renderSidebarList();

    // Перерисовываем основную рабочую область (сетку)
    renderMainGrid();

    // Показываем пользователю всплывающее уведомление с именем выбранного проекта
    showToast(p.name);
    

        
    // Запускаем таймер для выполнения перехода спустя 400 миллисекунд
    // Это позволяет интерфейсу отрисоваться и показать уведомление перед переходом
    setTimeout(() => {
        // По умолчанию предполагаем, что проект относится к типу fusion
        let baseUrl = '/';
        

        // Проверка типа проекта для выбора нужной страницы
        if (p.type === 'tis') {
            // Если тип проекта 'tis', используем index.html
            baseUrl = 'workspace';
        } else if (p.type === 'workflow') {
            // Если тип проекта 'workflow', используем workflow.html
            baseUrl = 'workflow';
        }
        // Выполняем переход на новую страницу с передачей ID проекта в URL
        // encodeURIComponent гарантирует корректную кодировку спецсимволов в ID
...
function createDefaultProject() {
    return {
        id: null, // ID будет сгенерирован на сервере или клиентом позже
        name: 'Новый проект',
        type: 'default',
        createdAt: Date.now(),
        lastUpdate: Date.now(),
        owner: 'current_user', // Заменить на реальное имя пользователя или ID
        settings: {
            theme: 'dark',
            defaultView: 'list',
            notifications: false
...
async function createNewProject() {
    ...
    newProj.id = newId;
    newProj.name = name;
    newProj.type = projectType.value;
    newProj.owner = displayUserNameSpan.innerText;
    newProj.order = maxOrder + 1;
...
function renderMainGrid() {
    ...
    filtered.forEach(project => {
        ...
        const options = {
        timeZone: 'Asia/Yekaterinburg', // Явный часовой пояс
        hour: '2-digit',
        minute: '2-digit',
        day: '2-digit',
        month: '2-digit',
