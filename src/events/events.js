...
function handleWheel(e) {
    if (e.ctrlKey) {
        ...
        if (deltaY > 0) {
            zoomOut(mouseX, mouseY); // Прокрутка вниз → масштабировать "на себя" (уменьшение)
        } else {
            zoomIn(mouseX, mouseY);  // Прокрутка вверх → масштабировать "от себя" (увеличение)
        }
        
        // Возвращаем true, чтобы предотвратить стандартное поведение
        return true;
    }
    // Если Ctrl не нажат, позволяем браузеру обрабатывать прокрутку
    // для перемещения по изображению через скроллбары
}
